...
int main(void)
{
    ...
    do {
        ...
        printf("40. test color+depth (1280X720@60, APC_DEPTH_DATA_11_BITS)\n");
        printf("41. test color       (1280X720@60, APC_DEPTH_DATA_11_BITS)\n");
        printf("42. test depth       (1280X720@60, APC_DEPTH_DATA_11_BITS)\n");
        printf("43. test color+depth (1280X720@60, APC_DEPTH_DATA_11_BITS)(thread)\n");
        printf("44. test color+depth (1280X720@30, APC_DEPTH_DATA_11_BITS)\n");
        printf("45. test color       (1280X720@30, APC_DEPTH_DATA_11_BITS)\n");
        printf("46. test depth       (1280X720@30, APC_DEPTH_DATA_11_BITS)\n");
        printf("47. test color+depth (1280X720@30, APC_DEPTH_DATA_11_BITS)(thread)\n");
        printf("48. test color+depth (1104x848@60, APC_DEPTH_DATA_14_BITS)(thread)\n");
#endif
...
#endif
        printf("60. test color+depth (1280X720@60, APC_DEPTH_DATA_11_BITS)(save file)\n");
        printf("61. test color+depth (1280X720@30, APC_DEPTH_DATA_11_BITS)(save file)\n");
        printf("255. exit)\n");
        scanf("%d", &input);
        switch(input) {
#if defined(_ENABLE_INTERACTIVE_UI_)
...
static void *test_color_time_stamp(void *arg)
{
    ...
    while (mCount < 1)
    {
        ret = APC_GetColorImageWithTimestamp(EYSD, GetCompositeDevSelectIndexPtr(),
...
static void *test_depth_time_stamp(void *arg)
{
    ...
    while (mCount < 1)
    {
        ret = APC_GetDepthImageWithTimestamp(EYSD, GetCompositeDevSelectIndexPtr(), (BYTE*)gDepthImgBuf, &gDepthImgSize, &cur_serial_num, gDepthDataType, &cur_tv_sec, &cur_tv_usec);
        ...
        if (ret == APC_OK) {
            ...
            if (bFirstReceived) {
                ...
                CT_DEBUG("[%s]SN: [%03d/%03d], SN_DIFF: [%03d], TS: [%lu], TS_DIFF: [%lu]\n", pre_str,
                       (int)cur_serial_num, serial_number, s_diff, (cur_tv_sec * 1000000 + cur_tv_usec), diff);
                if (RegisterSettings::DM_Quality_Register_Setting(EYSD, GetCompositeDevSelectIndexPtr(), g_pDevInfo[gsDevSelInfo.index].wPID) == 0) {
                    bAfterQCFG = true;
                }
...
static void *pthread_saving_depth(void *param) {
    ...
    while (mCount < 1)
    {
        ret = APC_GetDepthImageWithTimestamp(EYSD, GetCompositeDevSelectIndexPtr(),
...
static void *pthread_saving_color(void *param) {
    ...
    while (mCount < 1)
    {
        ...
        if ((ret == APC_OK) /*&& (gColorSerial > 0)*/) {
            ...
            int convertRGBRet = APC_ColorFormat_to_RGB24(EYSD, GetCompositeDevSelectIndexPtr(), writingBuffer, gColorImgBuf, gColorImgSize,
                                                         gColorWidth, gColorHeight, imageType);
            // 000 Convert from to RGB color
            // int convertBGRRet = APC_ColorFormat_to_BGR24(EYSD, GetCompositeDevSelectIndexPtr(), writingBuffer, gColorImgBuf, gColorImgSize,
            //                                                 gColorWidth, gColorHeight, imageType);
...
static int init_device(bool is_select_dev)
{
    ...
    for(i = 0; i < nDevCount; i++) {
        ...
        APC_GetDeviceInfo(EYSD, GetCompositeDevSelectIndexPtr(), g_pDevInfo + i);
        ...
        int nActualLength = 0;
        if (APC_OK == APC_GetFwVersion(EYSD, GetCompositeDevSelectIndexPtr(), FWVersion, 256, &nActualLength)) {
    ...
    for (i = SIMPLE_DEV_START_IDX; i < (SIMPLE_DEV_START_IDX + nDevSimpleCount); i++) {
        CT_DEBUG("Select Index = %d\n", i);
        gsDevSelInfo.index = i;
        APC_GetDeviceInfo(EYSD, GetSimpleDevSelectIndexPtr(), g_pDevInfo + i);
        CT_DEBUG("Device Name = %s\n", g_pDevInfo[i].strDevName);
        CT_DEBUG("PID = 0x%04x\n", g_pDevInfo[i].wPID);
        CT_DEBUG("VID = 0x%04x\n", g_pDevInfo[i].wVID);
        CT_DEBUG("ChipID = 0x%x\n", g_pDevInfo[i].nChipID);
        ...
        int nActualLength = 0;
        if (APC_OK == APC_GetFwVersion(EYSD, GetSimpleDevSelectIndexPtr(), FWVersion, 256, &nActualLength)) {
            CT_DEBUG("FW Version = %s\n", FWVersion);
            strcpy(devBuf, &g_pDevInfo[i].strDevName[strlen("/dev/")]);
            sprintf(devBuf_v4l, "/sys/class/video4linux/%s/name", devBuf);
            get_product_name(devBuf_v4l, devBuf_name);
...
static int open_device_default(bool two_open, int colorWidth, int colorHeight, int depthWidth, int depthHeight, int fps, WORD videoMode, bool blkMode)
{
    ...
    {
        if (APC_SetupBlock(EYSD, GetCompositeDevSelectIndexPtr(), blkMode) != 0) {
        ...
        if (gDepthWidth != 0) {
            ...
            ret = APC_SetDepthDataType(EYSD, GetCompositeDevSelectIndexPtr(), gDepthDataType); //4 ==> 11 bits
            if (ret == APC_OK) {
    ...
    if (APC_OK != APC_Setup_v4l2_requestbuffers(EYSD, GetCompositeDevSelectIndexPtr(), g_v4l2_buffer_quque_size)) {
        CT_DEBUG("APC_Setup_v4l2_requestbuffers failed\n");
    }

    if (two_open) {
        ret = APC_OpenDevice2(EYSD,
                              GetCompositeDevSelectIndexPtr(), gColorWidth,
        ...
        ret= APC_OpenDevice(EYSD, GetCompositeDevSelectIndexPtr(),
...
static int open_device(void)
{
    ...
    //e:[eys3D] 20200610 implement hypatia config
        ...
        if (input[0] == 'Y' || input[0] == 'y') {
            if (APC_SetupBlock(EYSD, GetCompositeDevSelectIndexPtr(), true) != 0) {
                CT_DEBUG("setup Blocking Failed\n");
            }
        } else {
            if (APC_SetupBlock(EYSD, GetCompositeDevSelectIndexPtr(), false) != 0) {
                CT_DEBUG("setup Blocking Failed\n");
            }
        }
    ...
    ret = APC_OpenDevice2(EYSD, GetCompositeDevSelectIndexPtr(), gColorWidth, gColorHeight, (bool)gColorFormat, gDepthWidth, gDepthHeight, gDepth_Transfer_ctrl, false, NULL, &gActualFps, IMAGE_SN_SYNC);
    if (ret == APC_OK) {
        CT_DEBUG("APC_OpenDevice2() success! (FPS = %d)\n", gActualFps);
    } else {
        CT_DEBUG("APC_OpenDevice2() fail.. (ret = %d)\n", ret);
...
static void *pfunc_thread_color(void *arg) {
    ...
    while (bTesting_color == true && mCount < TEST_RUN_NUMBER) {
        ...
        usleep(1000 * 5);
        /* APC_GetColorImage */
        ret = APC_GetColorImage(EYSD, GetCompositeDevSelectIndexPtr(), (BYTE*)gColorImgBuf, &gColorImgSize, &gColorSerial);
        ...
        if (ret == APC_OK && gColorSerial > 0) {
            ...
            if (cnt_frme > 99) {
                ...
                if (snapShot_color == true ) {
                    ...
                    ret = APC_ColorFormat_to_RGB24(EYSD, GetCompositeDevSelectIndexPtr(), gColorRGBImgBuf, gColorImgBuf, gColorImgSize,
                                                   gColorWidth, gColorHeight,
...
static void *pfunc_thread_depth(void *arg) {
    ...
    while (bTesting_depth == true && mCount < TEST_RUN_NUMBER) {
        ...
        usleep(1000 * 5);
        ret = APC_GetDepthImage(EYSD, GetCompositeDevSelectIndexPtr(), (BYTE*)gDepthImgBuf, &gDepthImgSize, &gDepthSerial, gDepthDataType);
        if (gDepthImgSize > m_BufferSize)
        {
            CT_DEBUG("Alloc size: %lu, but get depth size: %lu, check FW and close the application.\n", m_BufferSize, gDepthImgSize);
            goto EXIT;
    ...
    while (count < max_count) {
       ...
       if (ret != APC_OK) {
        ...
        if (ret == APC_OK) {
            if (bFirstReceived) {
                bFirstReceived = false;
                RegisterSettings::DM_Quality_Register_Setting(EYSD, GetCompositeDevSelectIndexPtr(), g_pDevInfo[gsDevSelInfo.index].wPID);
            }
            ret = APC_ColorFormat_to_RGB24(EYSD, GetCompositeDevSelectIndexPtr(), gColorRGBImgBuf, gColorImgBuf, gColorImgSize,
                                             gColorWidth, gColorHeight,
                                             gColorFormat ? APCImageType::Value::COLOR_MJPG : APCImageType::Value::COLOR_YUY2);
            // convert_yuv_to_rgb_buffer(gColorImgBuf, gColorRGBImgBuf, gColorWidth, gColorHeight);
            ret = getPointCloud(EYSD, GetCompositeDevSelectIndexPtr(), gColorRGBImgBuf, gColorWidth, gColorHeight,
...
static void setupFWRegister_EX8038()
{
    ...
    if (APC_OK != APC_SetFWRegister(EYSD, GetCompositeDevSelectIndexPtr(), 0xf0, 0x0d, flag)) {
...
static void setupFWRegister()
{
    ...
    if (APC_OK != APC_SetFWRegister(EYSD, GetCompositeDevSelectIndexPtr(), addr, value, flag)) {
        CT_DEBUG("APC_SetFWRegister failed\n");
    }
}
...
static void readFWRegister(void)
{
    ...
    if (APC_OK != APC_GetFWRegister(EYSD, GetCompositeDevSelectIndexPtr(), addr, (unsigned short *)&value, flag)) {
...
static void SetCounterMode()
{
    ...
    if (APC_OK != APC_SetControlCounterMode(EYSD, GetCompositeDevSelectIndexPtr(), (unsigned char)nValue)) {
...
static void GetCounterMode()
{
    ...
    if (APC_OK != APC_GetControlCounterMode(EYSD, GetCompositeDevSelectIndexPtr(), &nValue)) {
...
static void setIRValue()
{
    ...
    ret = APC_GetFWRegister(EYSD, GetCompositeDevSelectIndexPtr(),
...
static void Read3X(void)
{
    ...
    {
        if (APC_OK == APC_GetYOffset(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
        {
            printf("\n Read3%d\n", index);
...
static void Write3X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        if (APC_OK == APC_GetYOffset(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
        {
            if (APC_OK == APC_SetYOffset(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
            {
...
static void Read4X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        if (APC_OK == APC_GetRectifyTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
        {
            printf("\n Read4%d\n", index);
            for (int i = 0; i < nbfferLength; i++)
            {
...
static void Write4X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        if (APC_OK == APC_GetRectifyTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
        {
            if (APC_OK == APC_SetRectifyTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
            {
...
static void Read5X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        ...
        if (APC_OK == APC_GetZDTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, &zdTableInfo))
        {
            printf("\n Read5%d\n", index);
            for (int i = 0; i < nbfferLength; i++)
            {
...
static void Write5X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        ...
        if (APC_OK == APC_GetZDTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, &zdTableInfo))
        {
            if (APC_OK == APC_SetZDTable(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, &zdTableInfo))
            {
...
static void Read24X(void)
{
    ...
    for (index = 0; index <= 9; index++)
    {
        if (APC_OK == APC_GetLogData(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index, ALL_LOG))
        {
            printf("\n Read24%d ALL_LOG\n", index);
            for (int i = 0; i < nbfferLength; i++)
            {
...
static void ResetUNPData(void)
{
    ...
    ret = APC_ResetUNPData(EYSD, GetCompositeDevSelectIndexPtr());
    if (ret != APC_OK) {
...
static void ReadPlugIn(void)
{
    ...
    ret = APC_ReadFlashData(EYSD, GetCompositeDevSelectIndexPtr(), PLUGIN_ONLY, pBuffer, BufferLength, &ActualLength);
    if (ret != APC_OK) {
...
static void *pfunc_thread_point_cloud_fps(void *arg) {
    ...
    getPointCloudInfo(EYSD, GetCompositeDevSelectIndexPtr(), &pointCloudInfo, gDepthDataType, gDepthHeight);
    if (gColorImgBuf == NULL) {
        gColorImgBuf = (unsigned char*) calloc(2 * gColorWidth * gColorHeight, sizeof(unsigned char));
    }
    if (gColorImgBuf == NULL) {
    ...
    for (unsigned mCount = 0, pointCloudCount = 0; mCount < kMaxCount; mCount++) {
        // 000. Get Color Frame
        ret = APC_GetColorImageWithTimestamp(EYSD, GetCompositeDevSelectIndexPtr(), gColorImgBuf, &gColorImgSize, &cur_color_serial_num, 0,
...
//                }
        ...
        APC_ColorFormat_to_BGR24(EYSD, GetCompositeDevSelectIndexPtr(), rgbBuffer, gColorImgBuf, gColorImgSize, gColorWidth, gColorHeight,
                                 APCImageType::Value::COLOR_YUY2);
        // 002. Get Depth Frame
       ret = APC_GetDepthImageWithTimestamp(EYSD, GetCompositeDevSelectIndexPtr(), gDepthImgBuf, &gDepthImgSize, &cur_depth_serial_num, 0,
...
//                    (int)cur_depth_serial_num, s_depth_diff, (cur_depth_tv_sec * 1000000 + cur_depth_tv_usec), depth_diff);
        ...
        pointCloudFail = APC_GetPointCloud(EYSD, GetCompositeDevSelectIndexPtr(), rgbBuffer, gColorWidth, gColorHeight, gDepthImgBuf, gDepthWidth, gDepthHeight,
...
static void Write24X()
{
    ...
    for (index = 0; index <= 9; index++)
    {
        if (APC_OK == APC_GetLogData(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index, ALL_LOG))
        {
            if (APC_OK == APC_SetLogData(EYSD, GetCompositeDevSelectIndexPtr(), data, nbfferLength, &pActualLength, index))
            {
    ...
    if (isRGBNamed) {
        switch(type) {
            case 0: // Color stream (YUYV)
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"color_img_%02d_%d_%s.yuv", gsDevSelInfo.index, yuv_index++, DateTime);
                break;
            case 1: // Color stream (MJPEG)
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"color_img_%02d_%d_%s.jpg", gsDevSelInfo.index, mjpeg_index++, DateTime);
                break;
            case 2: // Depth stream
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"depth_img_%02d_%d_%s.yuv", gsDevSelInfo.index, depth_index++, DateTime);
            default:
                break;
        }
    } else {
        switch(type) {
            case 0: // YUV
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"color_yuv2rgb_%02d_%d_%s.raw", gsDevSelInfo.index, yuv_rgb_index++, DateTime);
                break;
            case 1: // MJPEG
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"color_mjpeg2rgb_%02d_%d_%s.raw", gsDevSelInfo.index, mjpeg_rgb_index++, DateTime);
                break;
            case 2: // Depth stream
                snprintf(fname, sizeof(fname), SAVE_FILE_PATH"depth_imgrgb_%02d_%d_%s.raw", gsDevSelInfo.index, depth_rgb_index++, DateTime);
                break;
            default:
                break;
        }
...
static int TransformDepthDataType(int *nDepthDataType, bool bRectifyData)
{
    ...
    if (((gCameraPID == APC_PID_8036) || (gCameraPID == APC_PID_MIPI_8036) || (gCameraPID == APC_PID_8052)) && (gDepthWidth == 640 && gDepthHeight == 360)) {
...
static void *pfunc_thread_close(void *arg)
{
    ...
    while (1) {
        sleep(1);
        if (EYSD) {
            ret = APC_CloseDevice(EYSD, GetCompositeDevSelectIndexPtr());
            if (ret == APC_OK) {
                CT_DEBUG("APC_CloseDevice() success!\n");
            } else {
                CT_DEBUG("APC_CloseDevice() fail.. (ret = %d)\n", ret);
...
int setupIR(unsigned short IRvalue)
{
    ...
    if (m_nIRValue != 0) {
        ret = APC_SetIRMode(EYSD, GetCompositeDevSelectIndexPtr(), 0x63); // 6 bits on for opening both 6 ir
        if (APC_OK != ret) return ret;
        CT_DEBUG("enable IR and set IR Value: %d\n", m_nIRValue);
        ret = APC_SetCurrentIRValue(EYSD, GetCompositeDevSelectIndexPtr(), m_nIRValue);
        if (APC_OK != ret) return ret;
        ret = APC_GetCurrentIRValue(EYSD, GetCompositeDevSelectIndexPtr(), &m_nIRValue);
        if (APC_OK != ret) return ret;
        CT_DEBUG("get IR Value: %d\n", m_nIRValue);
    } else {
        ret = APC_SetCurrentIRValue(EYSD, GetCompositeDevSelectIndexPtr(), m_nIRValue);
        if (APC_OK != ret) return ret;
        ret = APC_SetIRMode(EYSD,GetCompositeDevSelectIndexPtr(), 0x00); // turn off ir
        if (APC_OK != ret) return ret;
        CT_DEBUG("disable IR\n");
    }
    return APC_OK;
...
static void *property_bar_test_func(void *arg)
{
    ...
    ret = APC_GetCTPropVal(EYSD, GetCompositeDevSelectIndexPtr(), id, &cur_val);
    if (ret == APC_OK) {
        CT_DEBUG("AE[cur_val] = [0x%08x]\n", cur_val);
        //NOTE: The 0x01 means the 'EXPOSURE_MANUAL' 
        if (cur_val != 0x01) {
            set_val = 0x01;
            ret = APC_SetCTPropVal(EYSD, GetCompositeDevSelectIndexPtr(), id, set_val);
            if (ret != APC_OK) {
            ...
            ret = APC_GetCTRangeAndStep(EYSD, GetCompositeDevSelectIndexPtr(), id, &max, &min, &step, &def, &flag);
            if (ret == APC_OK) {
    ...
    ret = APC_GetPUPropVal(EYSD, GetCompositeDevSelectIndexPtr(), id, &cur_val);
    if (ret == APC_OK) {
    ...
    ret = APC_GetPURangeAndStep(EYSD, GetCompositeDevSelectIndexPtr(), id, &max, &min, &step, &def, &flag);
    if (ret == APC_OK) {
